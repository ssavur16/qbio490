...
Notice the @ and $ signs. What do each of them represent?
  ...
  @ represents: name of dataframe in maf_object
  $ represents: column names of dataframe
...
Refer to a codon chart and amino acid abbreviation chart to answer the following:
 ...
 A point/missense mutation at the third nucleotide of the codon could cause an S/R change. The codons AGU and AGC correspond to the amino acid serine (S), and if the third nucleotide were changed to an A or G, the resulting codon (i.e. AGA or AGG) would correspond to the amino acid arginine (R).
 
 - What about an A/T change?
 alanine, threonine
 A point/missense mutation at the first nucleotide of the codon could cuase an A/T chance. The codons GCU, GCC, GCA, and GCG correspond to the amino acid alanine (A), and if the first nucleotide underwent a transition change from guanine to adenine, the resulting codon (i.e. ACU, ACC, ACA, or ACG) would correspond to the amino acid threonine (T).
...
The number indicates the position of the protein, and the letter preceding and following this position correspond to the amino acids this position. P728A means that there is a variant that changes a Proline residue at position 728 of the protein to an Alanine residue. "fs" indicates that there is a frameshift mutation within this protein, and the amino acid at position 728 is the first affected by this change. *11 gives the position of the stop codon in the new reading frame; in the context of this problem, the translation termination codon is 11 residues from position 728.
...
ggsave("/Users/Owner/Documents/USC/Senior Year/qbio490/qbio_490_sonia/oncoplot_top#_SKCM.png")
```
...
```{r Exercise 8.2}
oncoplot(maf = maf_object,
         genes = c("TTN", "PCLO", "DNAH7", "RPTN", "HRNR"),
         borderCol = NA)
```
...
```{r Exercise 8.3}
age_diag_NA_mask <- !(maf_object@clinical.data$age_at_diagnosis == "[Not Available]")
maf_object@clinical.data = maf_object@clinical.data[age_diag_NA_mask, ]
...
ggsave("PATH/TO/qbio_490_NAME/coOncoplot_GENE_GENE_SKCM")
```   
