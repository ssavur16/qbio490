...
```{r}
write.csv(clinic, "/Users/Owner/Documents/USC/Senior Year/qbio490/qbio_490_sonia/analysis_data/skcm_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/Owner/Documents/USC/Senior Year/qbio490/qbio_490_sonia/analysis_data/skcm_clinical_data.csv")
```
...
```{r Exercise 3.1}
# viewing structure of data and first few lines:
str(clinic)
summary(clinic)
head(clinic)
...
# 2. what is the data type of $gender?
print(typeof(clinic$gender))

# 3. what is the age in years of the 5th patient?
print(as.numeric(clinic$birth_days_to[5]) / -365)

# 4. how old was the 2nd patient at their last follow up?
print((as.numeric(clinic$birth_days_to[2]) - as.numeric(clinic$last_contact_days_to[2])) / -365)
...
```{r Exercise 4.1}
# scatterplot of two numerical (continuous) variables
plot(height_cm_at_diagnosis ~ weight_kg_at_diagnosis, data = clinic,
     main = "height and weight of skin cancer patients",
     xlab = "height (cm) at diagnosis",
     ylab = "weight (kg) at diagnosis",
     col = "darkgreen")
```
...
boxplot(as.numeric(age_at_diagnosis) ~ race, data = clinic,
        main = "height and race of skin cancer patients",
        xlab = "race",
        ylab = "height (cm) at diagnosis")
```
...
```{r}
age_NA_mask <- !(clinic$age_at_diagnosis == "[Not Available]")
...
The y-axis represents the probability of survival for the patient at their age of time t.

# 2. What does the x axis represent?
The x-axis represents the time in days of the patient's age.

# 3. What age group appears to have the greatest rate of survival?
The "Young" age group appears to have the greatest rate of survival.

# 4. What age group appears to have the worst rate of survival? 
The "Old" age group appears to have the worst rate of survival.

# 5. What is the approx. 15-year survival rate for "Old" skin cancer patients?
The approximate 15-year survival rate (~5475 days) for "Old" skin cancer patients is 25%.
...
The p-value is 0.002, indicating statistical significance. This tells us that there is a statistically significant relationship between age and skin cancer survival. Skin cancer survival does decrease with increasing age beyond random chance.
